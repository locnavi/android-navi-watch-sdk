...
import com.locnavi.location.xunjimap.utils.Manufacturer;
...
public class StatusBarCompat {
    ...
    public static void setColor(Activity activity, int color, int statusBarAlpha) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (color == activity.getResources().getColor(R.color.ipsmap_colorPrimaryDark)) {
                setThirdManufacturerDarkMode(true, activity);
                activity.getWindow().getDecorView().setSystemUiVisibility(View.SYSTEM_UI_FLAG_LIGHT_STATUS_BAR);
            } else {
                setThirdManufacturerDarkMode(false, activity);
            }
    ...
    public static void setThirdManufacturerDarkMode(boolean darkmode, Activity activity) {
        setMiuiStatusBarDarkMode(darkmode, activity);
        setFlymeStatusBarDarkMode(darkmode, activity);
    }

    //miui 6.0+
    public static void setMiuiStatusBarDarkMode(boolean darkmode, Activity activity) {
        if (!Build.MANUFACTURER.equalsIgnoreCase(Manufacturer.XIAOMI)) {
            return;
        }
    ...
    //flyme
    public static void setFlymeStatusBarDarkMode(boolean dark, Activity activity) {
        if (!Build.MANUFACTURER.equalsIgnoreCase(Manufacturer.MEIZU)) {
            return;
        }
...
//        }
//        setTranslucentForImageView(activity, true);
//    }
    ...
    public static void setColorForDrawerLayout(Activity activity, DrawerLayout drawerLayout, int color, int statusBarAlpha) {
        ...
        if (!(contentLayout instanceof LinearLayout) && contentLayout.getChildAt(1) != null) {
            contentLayout.getChildAt(1).setPadding(0, DensityUtils.getStatusBarHeight(activity), 0, 0);
        }
        // 设置属性
    ...
    public static StatusBarView createStatusBarView(Activity activity, int color) {
        ...
        LinearLayout.LayoutParams params =
                new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, DensityUtils.getStatusBarHeight(activity));
     ...
     * @param activity 需要设置的Activity
     */
    public static void setTranslucentForImageView(Activity activity, boolean darkMode) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            setThirdManufacturerDarkMode(darkMode, activity);
